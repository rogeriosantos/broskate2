      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight text-dark-900 font-display mb-4">
            Epic{' '}
            <span className="bg-gradient-to-r from-primary-600 to-neon bg-clip-text text-transparent">
              Skate Spots
            </span>
          </h1>
          ...
          <p className="text-xl text-dark-600 font-medium max-w-2xl mx-auto mb-8">
            {location 
              ? 'Find legendary spots near you with GPS precision' 
              : 'Discover amazing skate spots from around the globe'
            }
          </p>
          ...
          {isAuthenticated ? (
            <Link 
              href="/spots/add"
              className="btn-primary px-8 py-4 shadow-2xl shadow-primary-500/25 group"
            >
              ...
              >
                ...
                {SPOT_TYPES.map(type => (
                  <option key={type} value={type}>
                    {type === 'street' && '🏙️ '}{type === 'park' && '🏞️ '}{type === 'bowl' && '🥣 '}{type === 'vert' && '📐 '}{type === 'mini_ramp' && '🛹 '}{type === 'ledge' && '📏 '}{type === 'stair' && '🪜 '}{type === 'rail' && '🚃 '}{type === 'gap' && '↗️ '}
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`px-3 py-1 text-xs font-bold rounded-full shadow-sm ${getDifficultyColor(spot.difficulty_level)} group-hover:scale-105 transition-transform duration-300`}>
                        {spot.difficulty_level === 1 && '🟢'} {spot.difficulty_level === 2 && '🔵'} {spot.difficulty_level === 3 && '🟡'} {spot.difficulty_level === 4 && '🟠'} {spot.difficulty_level === 5 && '🔴'}
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    <span className="bg-gradient-to-r from-primary-100 to-primary-200 text-primary-800 px-3 py-1 text-sm font-semibold rounded-full shadow-sm">
                      {spot.spot_type === 'street' && '🏙️'} {spot.spot_type === 'park' && '🏞️'} {spot.spot_type === 'bowl' && '🥣'} {spot.spot_type === 'vert' && '📐'} {spot.spot_type === 'mini_ramp' && '🛹'}
                      {' ' + spot.spot_type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                    </span>
                  </div>
                  
                  {spot.description && (
                    <p className="text-dark-600 mb-4 leading-relaxed line-clamp-2 font-medium">{spot.description}</p>
                  )}
                  ...
                  {spot.features && spot.features.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {spot.features.slice(0, 3).map((feature, featureIndex) => (
                        <span key={featureIndex} className="bg-gradient-to-r from-neon/10 to-glow/10 text-neon border border-neon/20 px-3 py-1 text-xs font-semibold rounded-full">
                          ⚡ {feature}
                        </span>
                      ))}
